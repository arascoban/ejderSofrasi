...
- Tüm varlıklar için makale ara aşaması `LUNA_MAKALE_PLANI.md` içinde hazır. M1 kanıt paketi üretimi uygulandı: `editorial_work/evidence/` altında 403 etkin ID ve `editorial_work/coverage.json` kapsam raporu var. M2’de altı Türkçe pilot taslak `editorial_work/drafts/` altında, `manifest.json` ile birlikte ve `pilot_review/not_published/dry_run_only` durumunda üretildi; Supabase’e aktarılmadı. Sonraki adım pilot kaynak incelemesi ve M3 aktarım hazırlığıdır; G–I eski planları korunur.
