...
- F yerel kabulü: `npm run lint`, `npm run typecheck`, `npm run test` (81 Vitest + 6 export testi) ve Webpack üretim derlemesi 527/527 başarılı. Yerel tarayıcıda `map?era=present&entity=CON-0001`, `map?era=silver-god-1673&entity=CON-0001`, Helvanar wiki dönem bağlamı ve hızlı dönem geçişi doğrulandı. Yeni F commit'i ve Vercel canlı smoke kontrolü bu kaydın sonraki yayın adımıdır.
