...
- Tüm varlıklar için makale ara aşaması `LUNA_MAKALE_PLANI.md` içinde hazır. M1 kanıt paketi üretimi uygulandı: `editorial_work/evidence/` altında 403 etkin ID ve `editorial_work/coverage.json` kapsam raporu var. M2’de altı Türkçe pilot taslak `editorial_work/drafts/` altında, `manifest.json` ile birlikte ve `pilot_review/not_published/dry_run_only` durumunda üretildi; Supabase’e aktarılmadı. Owner incelemesi için canlı, salt-okunur `/editor/pilots` rotası eklendi; bu görünüm Supabase’e yazmaz veya public yayın yapmaz. Sonraki adım pilot kaynak incelemesi ve M3 aktarım hazırlığıdır; G–I eski planları korunur.
...
- E kabulü kapandı. `cb6bb4b` GitHub main'e gönderildi, Vercel Production success oldu; canlı `/search?q=col%20sehri`, `/search?q=NPC-0059` ve gemi+tür+dönem filtresi 200 ile beklenen sonuçları verdi. F kabulü de kapandı; sıradaki özellik G ve güncel yürütme sırası `LUNA_DEPLOY_TALIMATLARI.md`.
...
- F1–F4 tamamlandı: dönem görünümü artık seçili döneme göre kara şekillerini/işaretleri süzüyor; emekli `ISL-0002` kimliği dönem durumunu `CON-0001` üzerinden çözüyor; Helvanar için Gümüş Tanrısının 1673 yılı/Günümüz kanıtı genel durum verisinden geliyor. Harita ve wiki seçili varlık görünür değilse açıklama panelini koruyor; dönemli wiki olguları, dönemsiz bilgiler, makale ve görseller otomatik olarak başka döneme atanmış sayılmıyor. Onaylı dönem sanatı olmadığı için kullanıcıya açık taslak/eksik sanat durumu gösteriliyor. Dönem başına ayrı kamera oturumu ve Canvas anahtarı hızlı geçişte eski katmanların taşınmasını engelliyor.
- F kabulü: `npm run lint`, `npm run typecheck`, `npm run test` (81 Vitest + 6 export testi) ve Webpack üretim derlemesi 527/527 başarılı. Yerel tarayıcıda `map?era=present&entity=CON-0001`, `map?era=silver-god-1673&entity=CON-0001`, Helvanar wiki dönem bağlamı ve hızlı dönem geçişi doğrulandı. `e8e473b` GitHub main'e gönderildi; GitHub Vercel status'u “Deployment has completed / success” verdi. Canlı HTTPS'te cache-bust ile F aşaması başlığı, eksik sanat bildirimi ve Helvanar Günümüz kayıp paneli doğrulandı.
