...
- Ara iş **M: kaynaklara dayalı varlık makaleleri** başladı. M1 kanıt paketleri `editorial_work/evidence/` altında ve `editorial_work/coverage.json` ile 403 etkin ID'yi kapsıyor; `npm run editorial:evidence -- --check` deterministik çıktıyı doğrular. M2’nin altı Türkçe pilot taslağı `editorial_work/drafts/` ve `editorial_work/manifest.json` altında `pilot_review/not_published/dry_run_only` durumunda üretildi; Supabase’e aktarılmadı. Sonraki iş pilot kaynak incelemesi ve M3 aktarım hazırlığıdır; toplu taslak/yayın üretme. M, G/H/I'nin yerine geçmez.
