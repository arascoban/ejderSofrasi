...
- Ara iş **M: kaynaklara dayalı varlık makaleleri** başladı. M1 kanıt paketleri `editorial_work/evidence/` altında ve `editorial_work/coverage.json` ile 403 etkin ID'yi kapsıyor; `npm run editorial:evidence -- --check` deterministik çıktıyı doğrular. M2’nin altı Türkçe pilot taslağı `editorial_work/drafts/` ve `editorial_work/manifest.json` altında `pilot_review/not_published/dry_run_only` durumunda üretildi; Supabase’e aktarılmadı. Owner incelemesi için canlı `/editor/pilots` rotası salt-okunur olarak eklendi; bu rota Supabase’e yazmaz ve public wiki’ye yayın yapmaz. Sonraki iş pilot kaynak incelemesi ve M3 aktarım hazırlığıdır; toplu taslak/yayın üretme. M, G/H/I'nin yerine geçmez.
...
**M2 pilot kabulü — 22 Eylül 2026:** `scripts/build_m2_pilot_drafts.mjs` altı seçili ID için başlık dışı H2 bölümleri, Türkçe paragraflar, ID tabanlı iç bağlantılar ve cümle/paragraf kanıt defteri üretiyor. `tests/editorial/m2-pilot-drafts.test.ts` Tiptap belge sınırlarını, link ID’lerini, kaynak referanslarını, hash’leri ve `not_published` durumunu doğrular. M3’e geçmeden önce pilot metinlerinin kaynak karşılaştırması insan tarafından yapılmalıdır; editör/Supabase yazımı ve yayın bu ara aşamada yoktur.
